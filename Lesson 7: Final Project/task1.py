...
# structure
# { user1: [[isConnectedTo1, isConnectedTo2...isConnectedToN], [likesToPlay1, likesToPlay2...likesToPlayN]], user2: [....] }
def decode(s):
	words = s.split(' ')
	user = words[0]
	s = ' '.join(words[4:])
...
def create_data_structure(string_input):
	...
	for i in range(0, len(lines), 2):
		user, connected = decode(lines[i])
		user, plays = decode(lines[i+1])
		network[user] = [connected, plays]		
	return network
...
def get_connections(network, user):
	if user in network: return network[user][0]
	return None
...
def get_games_liked(network,user):
	if user in network: return network[user][1]
	return None
...
def add_connection(network, user_A, user_B):
	...
	if user_B in network[user_A][0]: return False
...
def add_new_user(network, user, games):
	if user not in network: network[user] = [[], games]
	return network
...
def get_secondary_connections(network, user):
	...
	for connection in network[user][0]:
		if connection in network:
			for connectionOfConnection in network[connection][0]:
				if connectionOfConnection not in result:
					result.append(connectionOfConnection)
	return result
...
def connections_in_common(network, user_A, user_B):
	...
	for connection in connections_A:
		if connection in connections_B: result += 1
	return result
...
def path_to_friend(network, user_A, user_B, path = None):
	...
	if user_A not in network: return None
	if user_B not in network: return None
	if path == None: path = [user_A]
	connections_A = get_connections(network, user_A)
	if user_B in connections_A: 
		path.append(user_B)
		return path
	for connection in connections_A:
		if connection in path: continue
		path.append(connection)
		found = path_to_friend(network, connection, user_B, path)
