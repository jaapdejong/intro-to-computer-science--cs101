...
def isReciprocal(graph, source, destination, depth):
	if depth == 0: return source == destination
	if source in graph[destination]: return True
	for node in graph[destination]:
		if isReciprocal(graph, source, node, depth-1): return True
	return False

def compute_ranks(graph, depth):
        ...
        for page in graph:
            ...
            for node in graph:
                if page in graph[node]:
                    if not isReciprocal(graph, node, page, depth):
